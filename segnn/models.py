...
from segnn.segnn_new import SEGNN, GNN
from equivariant_diffusion.utils import remove_mean, remove_mean_with_mask
import numpy as np


class SEGNN_dynamics_QM9(nn.Module):
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context):
        ...
        if self.mode == 'segnn_dynamics':
            h_final, x_final = self.segnn(h, x, edges, node_mask=node_mask, edge_mask=edge_mask)
        ...
        if torch.any(torch.isnan(vel)):
            print('Warning: detected nan, resetting SEGNN output to zero.')
...
class SEGNN_encoder_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf, out_node_nf,
                 n_dims, hidden_nf=64, device='cpu',
                 act_fn=torch.nn.SiLU(), n_layers=4, attention=False,
                 tanh=False, mode='segnn_dynamics', norm_constant=0,
                 inv_sublayers=2, sin_embedding=False, normalization_factor=100, aggregation_method='sum',
...
class SEGNN_decoder_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf, out_node_nf,
                 n_dims, hidden_nf=64, device='cpu',
                 act_fn=torch.nn.SiLU(), n_layers=4, attention=False,
                 tanh=False, mode='segnn_dynamics', norm_constant=0,
